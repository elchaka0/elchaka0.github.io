document.addEventListener("DOMContentLoaded", function() {
  ...
  function mostrarHerramienta(herramienta) {
    ...
    herramientaDiv.innerHTML = `
      ...
      <button class="editar-herramienta">Editar</button>
      <button class="borrar-herramienta">Borrar</button>
    `;
    herramientasDiv.prepend(herramientaDiv);

    herramientaDiv.querySelector('.borrar-herramienta').addEventListener('click', function() {
      if (confirm("¿Estás seguro de que quieres borrar esta herramienta?")) {
        borrarHerramienta(herramienta.titulo);
      }
    });

    herramientaDiv.querySelector('.editar-herramienta').addEventListener('click', function() {
  ...
  function borrarHerramienta(titulo) {
    let herramientas = JSON.parse(localStorage.getItem("herramientas")) || [];
    herramientas = herramientas.filter(herramienta => herramienta.titulo !== titulo);
    localStorage.setItem("herramientas", JSON.stringify(herramientas));
    window.location.reload();
  }

  function editarHerramienta(herramienta) {
    const nuevoTitulo = prompt("Edita el título:", herramienta.titulo);
    const nuevaDescripcion = prompt("Edita la descripción:", herramienta.descripcion);

    if (nuevoTitulo && nuevaDescripcion) {
      let herramientas = JSON.parse(localStorage.getItem("herramientas")) || [];
      herramientas = herramientas.map(h => {
        if (h.titulo === herramienta.titulo && h.descripcion === herramienta.descripcion) {
          return {
            ...h,
            titulo: nuevoTitulo,
